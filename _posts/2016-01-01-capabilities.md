...
The first line of the file (beginning with the comment delimiter '#') is a comment describing the format of each of the subsequent line. Each line after the first specifies a stream to play and can either be a live stream or a VOD/media file. The EMS will start with the first item in the list for playback and will move sequentially down the file. The _localStreamName_ value specifies either a live stream or the path (relative to one of the EMS media directories) of a media file. The fields _sourceOffset_ and _duration_ specify what part of the source stream to play and for how long. The values for _sourceOffset_ and _duration_ are defined specifically as follows:
